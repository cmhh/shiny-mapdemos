...
# tweak the selectInput function
selectInput1 <- function(inputId, label, choices,
                         selected = NULL, multiple = FALSE,
                         selectize = FALSE, width = NULL, size = NULL,
                         class = "col-md-6"){
  res <- selectInput(inputId, label, choices, selected, multiple, selectize,
                     width, size)
  
  res$attribs$class <- sprintf("form-group %s shiny-input-container", class)
  res
}

# selectInput("var", "Select variable:", choices = c("v1", "v2"))
# selectInput1("var", "Select variable:", choices = c("v1", "v2"))
